 ...
 *
 */
public final class RtclFormatTest extends test.RandTest {
	...
	public void testDecode(int numItems) throws IOException {
		...
		for(int row = 0; row < numItems; row++) {
			...
			item.getRcvd().add(new Code(alnum(8)));
			item.getSent().add(new RSTQ(randInt(600)));
			item.getSent().add(new Code(alnum(8)));
			items.add(item);
		}
