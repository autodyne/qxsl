...
public final class QxmlFormat extends BaseFormat {
	...
	private final class QxmlDecoder {
		...
		public QxmlDecoder(InputStream stream) throws IOException {
			...
			try {
				stream = new ReusableInputStream(stream);
		...
		/**
		 * ストリームから{@link Rcvd}を1件読み込みます。
		 * 
		 * @param e {@link Rcvd}の開始タグ
		 * @return 読み込まれた{@link Rcvd}
		 * 
		 * @throws Exception 文法エラーまたは入出力例外
		 */
		private Rcvd rcvd(StartElement e) throws Exception {
			Rcvd rcvd = new Rcvd();
			fields(rcvd, e);
			reader.nextCloseTag();
			return rcvd;
		...
		/**
		 * ストリームから{@link Sent}を1件読み込みます。
		 * 
		 * @param e {@link Sent}の開始タグ
		 * @return 読み込まれた{@link Sent}
		 * @throws Exception 文法エラーまたは入出力例外
		 */
		private Sent sent(StartElement e) throws Exception {
			Sent sent = new Sent();
			fields(sent, e);
			reader.nextCloseTag();
			return sent;
		...
		private void fields(Tuple n, StartElement e) throws Exception {
			...
			while(iterator.hasNext()) {
				...
				n.set(fields.cache(at.getName(), at.getValue()));
			}
		}
	}
	 ...
	 *
	 */
	private static final class ReusableInputStream extends InputStream {
	...
	private final class QxmlEncoder {
		...
		private void field(Field field) throws Exception {
			final QName qname = field.type();
			try {
				String at = fields.getFormat(qname).encode(field);
				streamWriter.writeAttribute(qname, at);
	...
	private static final class EventReader implements java.io.Closeable {
		...
		/**
		 * 指定した{@link InputStream}からの{@link EventReader}を構築します
		 * 
		 * @param stream 読み込むストリーム
		 * @throws XMLStreamException {@link EventReader}が構築できない場合
		 */
