...
(setq key-qxsl-sign ((access 'SIGN Qxsl)))
...
(defun qxsl-sign it (getf it key-qxsl-sign))
...
(defun set-qxsl-sign (it val) (setf it key-qxsl-sign val))
...
(defun transform (it fmt)
	(let new
		(case fmt
			("qxml" (transform-qxsl it))
			("adxs" (transform-adif it))
			("adis" (transform-adif it))
			("cqww" (transform-cqww it))
			("jarl" (transform-jarl it))
			("jswl" (transform-jswl it))
			("ctxt" (transform-ctxt it))
			("zall" (transform-zlog it))
			("zdos" (transform-zdos it))
