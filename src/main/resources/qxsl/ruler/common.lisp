...
(setq defun (syntax (name pars body)
	`(set ',name (compile (lambda ,pars ,body)))))
...
(setq defmacro (syntax (name pars body)
	`(set ',name (compile (syntax ,pars ,body)))))
...
; floor
(defun floor num
	(! setScale num 0 ! (: FLOOR java.math.RoundingMode)))
...
; ceiling
(defun ceiling num
	(! setScale num 0 ! (: CEILING java.math.RoundingMode)))
...
; round
(defun round num
	(setScale num 0 ! (: HALF_UP java.math.RoundingMode)))
